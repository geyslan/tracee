...
type eventConfig struct {
	submit uint64 // event should be submitted to userspace
	emit   uint64 // event should be emitted to the user
}
...
func GetEssentialEventsList() map[events.ID]eventConfig {
	...
	return map[events.ID]eventConfig{
		...
		events.CgroupMkdir:      {submit: ^uint64(0)},
		events.CgroupRmdir:      {submit: ^uint64(0)},
	}
}
