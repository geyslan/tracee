...
func NormalizeTimeArgs(eventID ID, args []trace.Argument, timeArgNames []string) error {
	for i := range timeArgNames {
		arg := GetArg(args, timeArgNames[i])
		if arg == nil {
			return errfmt.Errorf("couldn't find argument %s of event %s",
				timeArgNames[i],
				Core.GetDefinitionByID(eventID).GetName(),
			)
		}
		if arg.Value == nil {
			continue
		...
		if !ok {
			return errfmt.Errorf("argument %s of signal %s is not uint64, it is %T",
				timeArgNames[i],
				Core.GetDefinitionByID(eventID).GetName(),
				arg.Value,
			)
