...
func (s *DiagnosticService) GetMetrics(ctx context.Context, in *pb.GetMetricsRequest) (*pb.GetMetricsResponse, error) {
	...
	return &pb.GetMetricsResponse{
		...
		BPFPerfEventStats: bpfPerfEventStatsProto(stats.GetBPFPerfEventStats()),
	}, nil
}
...
// bpfPerfEventStatsProto converts BPFPerfEventStats to a slice of pb.BPFPerfEventStats
func bpfPerfEventStatsProto(stats metrics.BPFPerfEventStats) []*pb.BPFPerfEventStats {
	var result []*pb.BPFPerfEventStats
	...
	for id := range eventIDs {
		...
		result = append(result, &pb.BPFPerfEventStats{
			Id:       pb.EventId(id),
			Name:     evtDef.GetName(),
