...
### `install-tools.yaml`
**Purpose:** Installs development tools (Clang, Go, dependencies).

**What it does:**
- Installs distro-specific dependencies (if available)
- Installs Clang via `install-clang.sh`
- Installs Go via `install-golang.sh`
...
**What it does:**
- Creates `/tmp/vm-ready` marker file
- Schedules reboot to activate new kernel

**Used by:** All distro templates (ubuntu, centos, alpine)

**Variables used:**
- `${KERNEL_VERSION}` - For logging
